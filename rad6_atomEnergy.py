...
wb = xlsxwriter.Workbook("C:\\Users\\opbir\\Documents\\ML_trainingData_atomEnergy.xlsx");
    ...
    for line in f:
        if "Properties=species:S:1:pos:R:3:uff:R:3" in line:
            ...
            if line[atomEnergy_indx+3] == "0":
                ...
                atomEnergy1 = float(line[atomEnergy_indx+3:atomEnergy_indx+15]);
            atomEnergy.append(atomEnergy1);
            sheet.write(count2,1,atomEnergy1);
        ...
        for i in indx:
            ...
            for sp in gas.species():
                ...
                if findSpecies in formulaList:
                    ...
                    if reactantCoeff != 0:
                        sheet.write(i+1,location+1,atomEnergy[location_formula]*96491.5667);             # [J/mol]
                    if productCoeff != 0:
                        sheet.write(i+1,location+1,-1*atomEnergy[location_formula]*96491.5667);          # [J/mol]

                count = count + 1;

fileExcel = pd.ExcelFile("C:\\Users\\opbir\\Documents\\ML_trainingData_atomEnergy.xlsx");   

for sheetName in fileExcel.sheet_names[1:]:
    df = pd.read_excel("C:\\Users\\opbir\\Documents\\ML_trainingData_atomEnergy.xlsx",sheet_name = sheetName);
    df.fillna(0,inplace=True);

    df.to_excel('C:\\Users\\opbir\\Documents\\ML_trainingData_atomEnergy_'+sheetName[:-4]+'.xlsx');
    print(df);
